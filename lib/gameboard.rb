...
:   (   )|  | |  |(.-'(   |    |   (   )|  | |   
 `--'`-' '  `-'  `-`--'`-'`-'  '    `-' `--`-'   "
	...
	def turn
		player = @turn_number % 2 == 0 ? @player2 : @player1
	...
	def check_for_win
		return true if check_horizontal_wins
		return true if check_vertical_wins
		return true if check_diagonal_wins
		false
	end
	...
	def win
		winner = @turn_number % 2 == 0 ? @player1.name : @player2.name
